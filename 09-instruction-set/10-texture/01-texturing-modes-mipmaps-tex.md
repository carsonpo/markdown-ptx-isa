...
`.level` (lod explicit): Requires an additional 32-bit scalar argument, lod, which contains the LOD to fetch from. The type of lod follows `.ctype` (either `.s32` or `.f32`). Geometries `.2dms` and `.a2dms` are not supported in this mode.
...
// Example of cubemap array, unified texturing mode
