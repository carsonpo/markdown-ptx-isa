...
| | d.b3 source select | d.b2 source select | d.b1 source select | d.b0 source select |
|-|-------------------|-------------------|-------------------|-------------------|
...
`.relaxed` and `.acquire`: May be used with `.global`, `.shared` spaces, or with generic addressing where the address points to `.global` or `.shared` space. Cache operations are not allowed.
...
The `.v8` (`.vec`) qualifier is supported if `.type` is `.b32` or `.s32` or `.u32` or `.f32` AND State space is `.global` or with generic addressing where address points to `.global` state space.
